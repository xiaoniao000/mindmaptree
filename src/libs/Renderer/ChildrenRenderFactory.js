  ...
  return {
    createRenderStrategy: function (node, drawNode) {
      ...
      if (node.checkIsRootNode()) {
        ...
        strategy = new ChildrenRenderStrategy(
          new SecondAndMoreRender(drawNode)
        );
      }
      return strategy;
    },
...
AbstractRender.prototype.commonRender = function (father, children, direction) {
  ...
  childX =
    hfx +
    direction *
      (this.nodeXInterval + nodeShapeRelative.getSingleNodeWidth(father) / 2);
  ...
  forEach(children, function (child) {
    ...
    childY =
      startY +
      child.areaHeight / 2 -
      nodeShapeRelative.getSingleNodeHeight(child) / 2;
 ...
 * @param direction
 */
FirstRender.prototype.renderLessThanTwo = function (
  father,
  leftChildren,
  direction
) {
  ...
  forEach(leftChildren, function (child) {
    ...
    var childX =
      hfx +
      direction *
        (self.nodeXInterval + nodeShapeRelative.getSingleNodeWidth(father) / 2);
    var childY = hfy - direction * countFlag * self.littleNodeYInterval;

    //@workaround:如果为1，4象限的节点
    if (
      (direction == 1 && countFlag == 1) ||
      (direction == -1 && countFlag == -1)
    ) {
