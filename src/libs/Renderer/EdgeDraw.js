...
var EdgeDraw = function (edge) {
  ...
  //画根结点到第一层节点的曲线
  function drawCurve() {
    var x1 = (sourceBox.x + sourceBox.x2) / 2; //源盒子x轴的中心
    var y1 = (sourceBox.y + sourceBox.y2) / 2; //源盒子y轴的中心
    var x2 = //目标盒子的x轴的最左边
      (targetBox.x + targetBox.x2) / 2 -
      (target.direction * targetBox.width) / 2; //target.direction 在左边还是右边
    ...
    var edgePath = paper.path(
      Raphael.fullfill("M{x1},{y1}Q{x3},{y3},{x2},{y2}", pathPoints)
    );
    edgePath.attr({
      stroke: "#999",
      "stroke-width": 2,
  ...
  function drawThreePath() {
    ...
    var shortPath = paper.path(
      Raphael.fullfill("M{x1},{y1}L{x2},{y2}", {
        x1: xs,
        y1: ys,
        x2: xc,
        y2: yc,
    ...
    var targetUnderPath = paper.path(
      Raphael.fullfill("M{x1},{y1}L{x2},{y2}", {
        x1: xt1,
        y1: yt1,
        x2: xt2,
